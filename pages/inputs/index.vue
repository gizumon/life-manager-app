<template>
  <v-card>
    <v-toolbar dense height="36px">
      <template>
        <v-tabs v-model="selectedType" fixed-tabs>
          ...
          >
            <v-icon dense color="blue darken-2" class="mr-1">{{
              config.icon
            }}</v-icon>
            {{ config.name }}
          </v-tab>
    ...
    <v-tabs-items v-model="selectedType">
      ...
      >
        <v-card flat>
          ...
          <v-form class="test" v-model="valid">
            <v-container>
              <div v-for="input in config.inputs" :key="input.id">
                <MultiCheck
                  v-if="input && input.type === 'multi-check'"
                  :config="input"
                  v-model="input.model"
                />
                <Input
...
export default Vue.extend({
  ...
  data() {
    const ref = this.$db.ref('data');
    ref.on('value', (snapshot) => {
      console.log('ref on!!!', snapshot.val());
    });
    return {
      valid: false,
